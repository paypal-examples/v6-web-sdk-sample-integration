...
This example demonstrates how to integrate PayU payments using PayPal's v6 Web SDK. PayU is a leading online payment service provider in Central and Eastern Europe, particularly popular in Poland.
...
- Pay U one-time payment integration
...
1. **SDK Initialization**: Loads PayPal Web SDK with PayU components using a client ID fetched from the server's `/paypal-api/auth/browser-safe-client-id` endpoint
...
- Ensure the currency is PLN (Polish Złoty)
