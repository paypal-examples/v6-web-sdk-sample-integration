async function onPayPalWebSdkLoaded() {
  try {
    ...
    const isEligible = paymentMethods.isEligible("doku");
    if (isEligible) {
      setupAPMPayment(sdkInstance);
    } else {
      showMessage({ text: "DOKU is not eligible. Please ensure your buyer country is Indonesia and currency is IDR.", type: "error" });
    }
  } catch (error) {
...
function setupAPMPayment(sdkInstance) {
  try {
    const apmCheckout = sdkInstance.createDOKUOneTimePaymentSession({
...
function setupButtonHandler(apmCheckout) {
  const apmButton = document.querySelector("#doku-button");
