...
async function onPayPalWebSdkLoaded() {
  try {
    ...
    const testBuyerCountry = "LV"; // Latvia
    const currencyCode = "EUR"; // Latvia Banks uses EUR

    // Initialize PayPal SDK instance with Latvia Banks components
    ...
    // Check if Latvia Banks is eligible for the currency
    ...
    if (!isLatviaBanksEligible) {
      showMessage({
        text: "Latvia Banks is not eligible for the selected currency (EUR)",
        type: "error",
      });
      return;
    }
...
/**
 * Creates an order for Latvia Banks payment
...
/**
 * Sets up Latvia Banks payment fields and handlers
...
async function setupLatviaBanksPayment(sdkInstance, currencyCode) {
  ...
  const fullnameContainer = document.querySelector("#latviabanks-full-name");
