async function onPayPalWebSdkLoaded() {
  try {
    ...
    const sdkInstance = await window.paypal.createInstance({
      clientId,
      testBuyerCountry: "ID",
      components: ["gopay-payments"],
    });
    const paymentMethods = await sdkInstance.findEligibleMethods({ currencyCode: "IDR" });
    const isEligible = paymentMethods.isEligible("gopay");
    if (isEligible) {
      setupPayment(sdkInstance);
    } else {
      showMessage({ text: "GoPay is not eligible. Please ensure your buyer country is Indonesia and currency is IDR.", type: "error" });
    }
  } catch (error) {
...
function setupPayment(sdkInstance) {
  try {
    const apmCheckout = sdkInstance.createGopayOneTimePaymentSession({ onApprove: handleApprove, onCancel: handleCancel, onError: handleError });
...
function setupPaymentFields(apmCheckout) {
  ...
  const emailField = apmCheckout.createPaymentFields({ type: "email", value: "", style: { variables: { textColor: "#333333", colorTextPlaceholder: "#999999", fontFamily: "Verdana, sans-serif", fontSizeBase: "14px" } } });
  document.querySelector("#gopay-full-name").appendChild(fullNameField);
  document.querySelector("#gopay-email").appendChild(emailField);
}

function setupButtonHandler(apmCheckout) {
  const apmButton = document.querySelector("#gopay-button");
  ...
  apmButton.addEventListener("click", async () => {
    try {
      ...
      if (isValid) {
        await apmCheckout.start({ presentationMode: "popup" }, createOrderWithPhone(phoneData));
      } else {
...
function validatePhoneFields() {
  ...
  if (errors.length > 0) throw new Error(`The following fields are required: ${errors.join(", ")}`);
...
async function createOrderWithPhone(phoneData) {
  try {
    const response = await fetch("/paypal-api/checkout/orders/create-order-for-one-time-payment", {
      ...
      body: JSON.stringify({ currencyCode: "IDR", processingInstruction: "ORDER_COMPLETE_ON_PAYMENT_APPROVAL" }),
    });
    if (!response.ok) throw new Error("Failed to create order");
    const { id } = await response.json();
    return { orderId: id, phone: { nationalNumber: phoneData.phoneNationalNumber, countryCode: phoneData.phoneCountryCode } };
  } catch (error) {
