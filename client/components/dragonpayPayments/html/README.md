...
This example demonstrates how to integrate Dragonpay payments using PayPal's v6 Web SDK. Dragonpay is a popular local payment method in the Philippines.

## 🏗️ Architecture Overview

This sample demonstrates a complete Dragonpay integration flow:

1. Initialize PayPal Web SDK with the Dragonpay component
2. Check eligibility for Dragonpay payment method
3. Create Dragonpay payment session with required payment and phone fields
4. Validate customer information and phone number before initiating payment
5. Process payment through Dragonpay popup flow
6. Handle payment approval, cancellation, and errors

## Features

- Dragonpay one-time payment integration
- Full name, email, and phone field validation
- Popup payment flow
- Eligibility checking for Dragonpay
...
3. **Enable Dragonpay Payment**
   ...
   - Find **Dragonpay** in the payment methods and enable it
...
1. **Navigate to the Dragonpay demo directory:**

   ```bash
   cd client/components/dragonpayPayments/html
   ```
...
1. **SDK Initialization**: Loads PayPal Web SDK with Dragonpay components using a client token fetched from the server's `/paypal-api/auth/browser-safe-client-token` endpoint
2. **Eligibility Check**: Verifies Dragonpay is eligible for the merchant
3. **Session Creation**: Creates Dragonpay payment session with event callbacks for handling payment lifecycle events
4. **Field Setup**: Mounts the required full name, email, and phone fields
...
### Dragonpay not eligible

- Verify `testBuyerCountry` is set to "PH"
- Check that Dragonpay is enabled for the merchant in PayPal account settings

### Validation fails

- Ensure full name, email, and phone fields are properly mounted
