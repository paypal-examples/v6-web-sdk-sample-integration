...
This example demonstrates how to integrate Bizum payments using PayPal's v6 Web SDK. Bizum is Spain's most popular instant mobile payment service that allows Spanish users to make payments directly from their banking app.
...
5. Process payment through Bizum popup flow
6. Handle payment approval, cancellation, and errors

## Features

- Bizum one-time payment integration
- Full name and phone number field validation
- Popup payment flow
...
3. **Enable Bizum Payment**
   ...
   - Find **Bizum** in the payment methods and enable it
...
1. **Navigate to the Bizum demo directory:**

   ```bash
   cd client/components/bizumPayments/html
   ```
...
1. **SDK Initialization**: Loads PayPal Web SDK with Bizum components using a client ID fetched from the server's `/paypal-api/auth/browser-safe-client-id` endpoint
2. **Eligibility Check**: Verifies Bizum is eligible for the merchant
3. **Session Creation**: Creates Bizum payment session with event callbacks for handling payment lifecycle events
4. **Field Setup**: Mounts the required full name and phone number fields
...
- Verify `testBuyerCountry` is set to "ES" (Spain)
- Check that Bizum is enabled for the merchant in PayPal account settings
- Ensure the currency is EUR

### Validation fails

- Ensure full name and phone number fields are properly mounted with javascript
