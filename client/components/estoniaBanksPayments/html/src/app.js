async function onPayPalWebSdkLoaded() {
  try {
    ...
    const isEstoniaEligible = paymentMethods.isEligible("estonia_banks");

    if (isEstoniaEligible) {
      setupEstoniaBanksPayment(sdkInstance);
    } else {
      showMessage({
        text: "Estonia Banks is not eligible. Please ensure your buyer country is Estonia.",
        type: "error",
      });
...
function setupEstoniaBanksPayment(sdkInstance) {
  try {
    ...
    const estoniaCheckout = sdkInstance.createEstoniaOneTimePaymentSession({
    ...
    setupPaymentFields(estoniaCheckout);

    // Setup button click handler
    setupButtonHandler(estoniaCheckout);
...
function setupPaymentFields(estoniaCheckout) {
  ...
  const fullNameField = estoniaCheckout.createPaymentFields({
    type: "name",
  ...
  document.querySelector("#estonia-full-name").appendChild(fullNameField);
...
function setupButtonHandler(estoniaCheckout) {
  const estoniaButton = document.querySelector("#estonia-button");
  estoniaButton.removeAttribute("hidden");

  estoniaButton.addEventListener("click", async () => {
    try {
      console.log("Validating payment fields...");

      // Validate the payment fields (name)
      const isValid = await estoniaCheckout.validate();
...
// Create PayPal order for Estonia Banks with custom payload
async function createOrder() {
  try {
    console.log("Creating PayPal order for Estonia Banks...");

    // Custom payload required by Estonia Banks with processing_instruction
