...
This sample demonstrates a complete Estonia Banks integration flow:

1. Initialize PayPal Web SDK with the Estonia Banks component
2. Check eligibility for Estonia Banks payment method
3. Create Estonia Banks payment session with required payment fields
...
6. Authorize the payment through Estonia Banks popup flow
...
3. **Enable Estonia Banks Payment**
   ...
   - Find **Estonia Banks** in the payment methods and enable it
...
Estonia Banks is available in Estonia.

### Client-Side Flow

1. **SDK Initialization**: Loads PayPal Web SDK with the Estonia Banks component using a client ID fetched from the server's `/paypal-api/auth/browser-safe-client-id` endpoint. `testBuyerCountry` is set to `"EE"` for sandbox testing.
2. **Eligibility Check**: Verifies Estonia Banks is eligible for the merchant with EUR currency.
3. **Session Creation**: Creates an Estonia Banks payment session (`createEstoniaOneTimePaymentSession`) with event callbacks for handling payment lifecycle events.
...
6. **Order & Payment Flow**: Opens a popup window where the customer selects their bank and authorizes the payment. The order is created server-side via `/paypal-api/checkout/orders/create-order-for-one-time-payment` with the `ORDER_COMPLETE_ON_PAYMENT_APPROVAL` processing instruction.
...
Estonia Banks requires an order creation payload with:
...
### Estonia Banks not eligible

- Verify `testBuyerCountry` is set to `"EE"`
- Check that `currencyCode` is set to `"EUR"` (Euro)
- Ensure Estonia Banks is enabled for the merchant in PayPal account settings
...
- **Ensure currency_code is set to `"EUR"`** — this is critical for Estonia Banks
