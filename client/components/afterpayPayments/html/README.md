...
This example demonstrates how to integrate Afterpay payments using PayPal's v6 Web SDK. Afterpay is a popular "Buy Now, Pay Later" service widely used in Australia, New Zealand, the US, and the UK.
...
- AUD (Australian Dollar) currency support
...
3. **Enable Afterpay Payment**
   ...
   - Ensure your account is configured to accept **AUD (Australian Dollar)** currency
...
1. **SDK Initialization**: Loads PayPal Web SDK with Afterpay components using a client ID fetched from the server's `/paypal-api/auth/browser-safe-client-id` endpoint
...
- Verify `testBuyerCountry` is set to "AU" (Australia)
- Check that Afterpay is enabled for the merchant in PayPal account settings
- Ensure the currency is AUD (Australian Dollar)
