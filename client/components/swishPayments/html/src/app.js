  ...
  try {
    ...
    if (isSwishEligible) {
      ...
      renderAlert({
        type: "warning",
        message:
          "Swish is not eligible. Please ensure your buyer country is Sweden.",
      });
...
function setupSwishPayment(sdkInstance) {
  try {
    ...
    renderAlert({
      type: "danger",
      message: "Failed to set up Swish Payment Session",
    });
...
function setupPaymentFields(swishCheckout) {
  ...
  const phoneField = swishCheckout.createPaymentFields({
    type: "phone",
    value: "", // Optional prefill value
    style: {
      // Optional styling to match your website
  ...
  document.querySelector("#swish-phone").appendChild(phoneField);
}
