async function onPayPalWebSdkLoaded() {
  try {
    ...
    const sdkInstance = await window.paypal.createInstance({
      clientId,
      testBuyerCountry: "DE", // Germany for Paysafecard testing
      components: ["paysafecard-payments"],
    });

    // Check if Paysafecard is eligible
    ...
    if (isPaysafecardEligible) {
      ...
      console.error("Paysafecard is not eligible");
    }
  } catch (error) {
...
function setupPaysafecardPayment(sdkInstance) {
  try {
    ...
    const paysafecardCheckout =
      sdkInstance.createPaysafecardOneTimePaymentSession({
...
function setupButtonHandler(paysafecardCheckout) {
  ...
  paysafecardButton.addEventListener("click", async () => {
    try {
      ...
      const isValid = await paysafecardCheckout.validate();
...
async function createOrder() {
  try {
    console.log("Creating PayPal order for Paysafecard...");
