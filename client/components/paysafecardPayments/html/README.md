...
This sample demonstrates a complete Paysafecard integration flow:

1. Initialize PayPal Web SDK with the Paysafecard component
2. Check eligibility for Paysafecard payment method
3. Create Paysafecard payment session with required payment fields
...
6. Authorize the payment through Paysafecard popup flow
...
- Popup payment flow
- Eligibility checking for Paysafecard
...
3. **Enable Paysafecard Payment**
   ...
   - Find **Paysafecard** in the payment methods and enable it
...
Paysafecard is available in Europe and other regions.

### Client-Side Flow

1. **SDK Initialization**: Loads PayPal Web SDK with the Paysafecard component using a client ID fetched from the server's `/paypal-api/auth/browser-safe-client-id` endpoint. `testBuyerCountry` is set to `"DE"` for sandbox testing.
2. **Eligibility Check**: Verifies Paysafecard is eligible for the merchant with EUR currency.
3. **Session Creation**: Creates a Paysafecard payment session (`createPaysafecardOneTimePaymentSession`) with event callbacks for handling payment lifecycle events.
...
6. **Order & Payment Flow**: Opens a popup window where the customer enters their Paysafecard PIN to authorize the payment. The order is created server-side via `/paypal-api/checkout/orders/create-order-for-one-time-payment` with the `ORDER_COMPLETE_ON_PAYMENT_APPROVAL` processing instruction.
...
Paysafecard requires an order creation payload with:
...
### Paysafecard not eligible

- Verify `testBuyerCountry` is set to `"DE"` or another supported European country
- Check that `currencyCode` is set to `"EUR"` (Euro)
- Ensure Paysafecard is enabled for the merchant in PayPal account settings
...
- **Ensure currency_code is set to `"EUR"`** — this is critical for Paysafecard
