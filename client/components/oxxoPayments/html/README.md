# Oxxo One-Time Payment Integration

This example demonstrates how to integrate Oxxo payments using PayPal's v6 Web SDK. Oxxo is a popular cash payment method in Mexico, allowing customers to pay for online purchases at Oxxo convenience stores.

## 🏗️ Architecture Overview

This sample demonstrates a complete Oxxo integration flow:

1. Initialize PayPal Web SDK with the Oxxo component
2. Check eligibility for Oxxo payment method
3. Create Oxxo payment session with required payment fields
4. Validate customer information and expiry date before initiating payment
5. Process payment through Oxxo popup flow
6. Handle payment approval, cancellation, and errors

## Features

- Oxxo one-time payment integration
- Full name, email, and expiry date field validation
- Popup payment flow
- Eligibility checking for Oxxo
...
3. **Enable Oxxo Payment**
   ...
   - Find **Oxxo** in the payment methods and enable it
...
1. **Navigate to the Oxxo demo directory:**

   ```bash
   cd client/components/oxxoPayments/html
   ```
...
1. **SDK Initialization**: Loads PayPal Web SDK with Oxxo components using a client token fetched from the server's `/paypal-api/auth/browser-safe-client-token` endpoint
2. **Eligibility Check**: Verifies Oxxo is eligible for the merchant
3. **Session Creation**: Creates Oxxo payment session with event callbacks for handling payment lifecycle events
4. **Field Setup**: Mounts the required full name, email, and expiry date fields
...
- Check that Oxxo is enabled for the merchant in PayPal account settings

### Validation fails

- Ensure full name, email, and expiry date fields are properly mounted
