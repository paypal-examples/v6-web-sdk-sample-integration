...
6. Authorize the payment through MyBank popup flow
...
MyBank is available in Italy and other European countries.

### Client-Side Flow

1. **SDK Initialization**: Loads PayPal Web SDK with the MyBank component using a client ID fetched from the server's `/paypal-api/auth/browser-safe-client-id` endpoint. `testBuyerCountry` is set to `"IT"` for sandbox testing.
2. **Eligibility Check**: Verifies MyBank is eligible for the merchant with EUR currency.
3. **Session Creation**: Creates a MyBank payment session (`createMybankOneTimePaymentSession`) with event callbacks for handling payment lifecycle events.
