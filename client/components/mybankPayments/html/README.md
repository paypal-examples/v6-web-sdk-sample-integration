...
This example demonstrates how to integrate MyBank payments using PayPal's v6 Web SDK. MyBank is a popular online banking payment method in Italy.

## 🏗️ Architecture Overview

This sample demonstrates a complete MyBank integration flow:

1. Initialize PayPal Web SDK with the MyBank component
2. Check eligibility for MyBank payment method
3. Create MyBank payment session with required payment fields
4. Validate customer information before initiating payment
5. Process payment through MyBank popup flow
...
- Popup payment flow
- Eligibility checking for MyBank
...
3. **Enable MyBank Payment**
   ...
   - Find **MyBank** in the payment methods and enable it
...
1. **Navigate to the MyBank demo directory:**

   ```bash
   cd client/components/mybankPayments/html
   ```
...
1. **SDK Initialization**: Loads PayPal Web SDK with MyBank components using a client ID fetched from the server's `/paypal-api/auth/browser-safe-client-id` endpoint
2. **Eligibility Check**: Verifies MyBank is eligible for the merchant
3. **Session Creation**: Creates MyBank payment session with event callbacks for handling payment lifecycle events
...
- Verify `testBuyerCountry` is set to "IT" (Italy)
- Check that MyBank is enabled for the merchant in PayPal account settings
- Ensure the currency is EUR
