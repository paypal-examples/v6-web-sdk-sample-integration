async function onPayPalWebSdkLoaded() {
  try {
    ...
    const sdkInstance = await window.paypal.createInstance({
      clientId,
      testBuyerCountry: "CN", // China for WechatPay testing
      components: ["wechatpay-payments"],
    });

    // Check if WechatPay is eligible
    ...
    if (isWechatpayEligible) {
      ...
      console.error("WechatPay is not eligible");
    }
  } catch (error) {
  ...
  wechatpayButton.addEventListener("click", async () => {
    try {
      ...
      if (isValid) {
        ...
        await wechatpayCheckout.start({ presentationMode: "popup" }, createOrder());
      } else {
...
async function createOrder() {
  try {
    console.log("Creating PayPal order for WechatPay...");
