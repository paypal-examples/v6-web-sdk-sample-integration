...
This sample demonstrates a complete Skrill integration flow:

1. Initialize PayPal Web SDK with the Skrill component
2. Check eligibility for Skrill payment method
3. Create Skrill payment session with required payment fields
...
6. Authorize the payment through Skrill popup flow
...
- Popup payment flow
- Eligibility checking for Skrill
...
3. **Enable Skrill Payment**
   ...
   - Find **Skrill** in the payment methods and enable it
...
Skrill is available in Europe and other regions.

### Client-Side Flow

1. **SDK Initialization**: Loads PayPal Web SDK with the Skrill component using a client ID fetched from the server's `/paypal-api/auth/browser-safe-client-id` endpoint. `testBuyerCountry` is set to `"DE"` for sandbox testing.
2. **Eligibility Check**: Verifies Skrill is eligible for the merchant with EUR currency.
3. **Session Creation**: Creates a Skrill payment session (`createSkrillOneTimePaymentSession`) with event callbacks for handling payment lifecycle events.
...
6. **Order & Payment Flow**: Opens a popup window where the customer authorizes the payment through Skrill. The order is created server-side via `/paypal-api/checkout/orders/create-order-for-one-time-payment` with the `ORDER_COMPLETE_ON_PAYMENT_APPROVAL` processing instruction.
...
Skrill requires an order creation payload with:
...
- Ensure Skrill is enabled for the merchant in PayPal account settings
...
- **Ensure currency_code is set to `"EUR"`** — this is critical for Skrill
