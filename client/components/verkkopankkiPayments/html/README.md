...
This example demonstrates how to integrate Verkkopankki payments using PayPal's v6 Web SDK. Verkkopankki (Online Banking) is a widely used payment method in Finland that enables direct bank transfers.
...
5. Process payment through Verkkopankki popup flow
...
1. **Navigate to the Verkkopankki demo directory:**

   ```bash
   cd client/components/verkkopankkiPayments/html
   ```
...
1. **SDK Initialization**: Loads PayPal Web SDK with Verkkopankki components using a client ID fetched from the server's `/paypal-api/auth/browser-safe-client-id` endpoint
2. **Eligibility Check**: Verifies Verkkopankki is eligible for the merchant
3. **Session Creation**: Creates Verkkopankki payment session with event callbacks for handling payment lifecycle events
...
### Verkkopankki not eligible

- Verify `testBuyerCountry` is set to "FI" (Finland)
- Check that Verkkopankki is enabled for the merchant in PayPal account settings
