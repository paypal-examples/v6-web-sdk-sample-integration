  ...
  try {
    ...
    if (isVerkkopankkiEligible) {
      ...
      renderAlert({
        type: "warning",
        message:
          "Verkkopankki is not eligible. Please ensure your buyer country is Finland.",
      });
...
function setupVerkkopankkiPayment(sdkInstance) {
  try {
    ...
    renderAlert({
      type: "danger",
      message: "Failed to set up Verkkopankki Payment Session",
    });
...
function setupPaymentFields(verkkopankkiCheckout) {
  ...
  document
    .querySelector("#verkkopankki-full-name")
