...
6. Authorize the payment through Trustly popup flow
...
Trustly is available in Sweden and other European countries.

### Client-Side Flow

1. **SDK Initialization**: Loads PayPal Web SDK with the Trustly component using a client ID fetched from the server's `/paypal-api/auth/browser-safe-client-id` endpoint. `testBuyerCountry` is set to `"SE"` for sandbox testing.
2. **Eligibility Check**: Verifies Trustly is eligible for the merchant with SEK currency.
3. **Session Creation**: Creates a Trustly payment session (`createTrustlyOneTimePaymentSession`) with event callbacks for handling payment lifecycle events.
...
Trustly requires an order creation payload with:
...
- Ensure Trustly is enabled for the merchant in PayPal account settings
...
- **Ensure currency_code is set to `"SEK"`** — this is critical for Trustly
