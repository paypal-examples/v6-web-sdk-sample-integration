...
6. Authorize the payment through GoPay popup flow
...
3. **Enable GoPay** — In [sandbox.paypal.com](https://www.sandbox.paypal.com), go to **Account Settings** → **Payment methods**, enable GoPay, and configure **IDR** currency
...
GoPay is available in Indonesia (ID).

### Client-Side Flow

1. **SDK Initialization**: `testBuyerCountry` is `"ID"`, component `gopay-payments`.
2. **Eligibility Check**: `isEligible("gopay")` with IDR currency.
3. **Session Creation**: `createGopayOneTimePaymentSession`.
...
- Ensure GoPay is enabled in PayPal sandbox account settings
