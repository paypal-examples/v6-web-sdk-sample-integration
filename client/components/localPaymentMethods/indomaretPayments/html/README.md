...
6. Authorize the payment through Indomaret popup flow
...
3. **Enable Indomaret** — In [sandbox.paypal.com](https://www.sandbox.paypal.com), go to **Account Settings** → **Payment methods**, enable Indomaret, and configure **IDR** currency
...
Indomaret is available in Indonesia (ID).

### Client-Side Flow

1. **SDK Initialization**: `testBuyerCountry` is `"ID"`, component `indomaret-payments`.
2. **Eligibility Check**: `isEligible("indomaret")` with IDR currency.
3. **Session Creation**: `createIndomaretOneTimePaymentSession`.
...
- Ensure Indomaret is enabled in PayPal sandbox account settings
