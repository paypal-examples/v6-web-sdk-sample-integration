...
6. Authorize the payment through Alfamart popup flow
...
3. **Enable Alfamart** — In [sandbox.paypal.com](https://www.sandbox.paypal.com), go to **Account Settings** → **Payment methods**, enable Alfamart, and configure **IDR** currency
...
Alfamart is available in Indonesia (ID).

### Client-Side Flow

1. **SDK Initialization**: `testBuyerCountry` is set to `"ID"`, component `alfamart-payments`.
2. **Eligibility Check**: `isEligible("alfamart")` with IDR currency.
3. **Session Creation**: `createAlfamartOneTimePaymentSession`.
...
- Ensure Alfamart is enabled in PayPal sandbox account settings
