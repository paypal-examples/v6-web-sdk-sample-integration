...
This example demonstrates how to integrate Kredivo payments using PayPal's v6 Web SDK. Kredivo is a leading buy now pay later (BNPL) installment service in Indonesia that allows customers to make purchases and pay in flexible installments through their mobile app or web platform.

## 🏗️ Architecture Overview

This sample demonstrates a complete Kredivo integration flow:
...
5. Authorize the payment through Kredivo popup flow
...
- Popup payment flow with Kredivo authentication
...
3. **Enable Kredivo Payment**
   ...
   - Find **Kredivo** in the payment methods and enable it
...
1. **Navigate to the Kredivo demo directory:**

   ```bash
   cd client/components/kredivoPayments/html
   ```
...
1. **SDK Initialization**: Loads PayPal Web SDK with Kredivo components using a client token fetched from the server's `/paypal-api/auth/browser-safe-client-token` endpoint
2. **Eligibility Check**: Verifies Kredivo is eligible for the merchant with IDR currency
3. **Session Creation**: Creates Kredivo payment session with event callbacks for handling payment lifecycle events
...
6. **Payment Flow**: Opens a popup window where customers authenticate with Kredivo and select their preferred installment plan. The order is created server-side via `/paypal-api/checkout/orders/create` before displaying the popup
...
- **Ensure currency_code is set to "IDR"** - this is critical for Kredivo
