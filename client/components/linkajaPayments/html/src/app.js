async function onPayPalWebSdkLoaded() {
  try {
    ...
    const sdkInstance = await window.paypal.createInstance({
      clientId,
      testBuyerCountry: "ID",
      components: ["linkaja-payments"],
    });
    const paymentMethods = await sdkInstance.findEligibleMethods({ currencyCode: "IDR" });
    const isEligible = paymentMethods.isEligible("linkaja");
    if (isEligible) {
      setupAPMPayment(sdkInstance);
    } else {
      showMessage({ text: "LinkAja is not eligible. Please ensure your buyer country is Indonesia and currency is IDR.", type: "error" });
    }
  } catch (error) {
...
function setupAPMPayment(sdkInstance) {
  try {
    const apmCheckout = sdkInstance.createLinkajaOneTimePaymentSession({
...
function setupPaymentFields(apmCheckout) {
  ...
  document.querySelector("#linkaja-full-name").appendChild(fullNameField);
  document.querySelector("#linkaja-email").appendChild(emailField);
}

function setupButtonHandler(apmCheckout) {
  const apmButton = document.querySelector("#linkaja-button");
