...
6. Authorize the payment through Twint popup flow
...
Twint is available in Switzerland.

### Client-Side Flow

1. **SDK Initialization**: Loads PayPal Web SDK with the Twint component using a client ID fetched from the server's `/paypal-api/auth/browser-safe-client-id` endpoint. `testBuyerCountry` is set to `"CH"` for sandbox testing.
2. **Eligibility Check**: Verifies Twint is eligible for the merchant with CHF currency.
3. **Session Creation**: Creates a Twint payment session (`createTwintOneTimePaymentSession`) with event callbacks for handling payment lifecycle events.
...
6. **Order & Payment Flow**: Opens a popup window where the customer authorizes the payment in their Twint app. The order is created server-side via `/paypal-api/checkout/orders/create-order-for-one-time-payment` with the `ORDER_COMPLETE_ON_PAYMENT_APPROVAL` processing instruction.
...
- `POST /paypal-api/checkout/orders/create-order-for-one-time-payment` - Create order with CHF currency and `ORDER_COMPLETE_ON_PAYMENT_APPROVAL` processing instruction
- `GET /paypal-api/checkout/orders/:orderId` - Fetch order details after approval

### Special Order Configuration

Twint requires an order creation payload with:
...
- Verify `testBuyerCountry` is set to `"CH"`
- Check that `currencyCode` is set to `"CHF"` (Swiss Franc)
- Ensure Twint is enabled for the merchant in PayPal account settings
- Verify your PayPal account is configured to accept CHF currency
...
- **Ensure currency_code is set to `"CHF"`** — this is critical for Twint
