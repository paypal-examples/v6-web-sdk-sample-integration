# Boletobancario One-Time Payment Integration

This example demonstrates how to integrate Boletobancario payments using PayPal's v6 Web SDK. Boletobancario is a popular cash payment method in Brazil, allowing customers to pay for online purchases at banks or convenience stores.

## 🏗️ Architecture Overview

This sample demonstrates a complete Boletobancario integration flow:

1. Initialize PayPal Web SDK with the Boletobancario component
2. Check eligibility for Boletobancario payment method
3. Create Boletobancario payment session with required payment, billing, and tax fields
4. Validate customer information, billing address, tax info, and expiry date before initiating payment
5. Process payment through Boletobancario popup flow
6. Handle payment approval, cancellation, and errors

## Features

- Boletobancario one-time payment integration
- Full name, email, billing address, tax info, and expiry date field validation
- Popup payment flow
- Eligibility checking for Boletobancario
...
3. **Enable Boletobancario Payment**
   ...
   - Find **Boletobancario** in the payment methods and enable it
...
1. **Navigate to the Boletobancario demo directory:**

   ```bash
   cd client/components/boletobancarioPayments/html
   ```
...
1. **SDK Initialization**: Loads PayPal Web SDK with Boletobancario components using a client token fetched from the server's `/paypal-api/auth/browser-safe-client-token` endpoint
2. **Eligibility Check**: Verifies Boletobancario is eligible for the merchant
3. **Session Creation**: Creates Boletobancario payment session with event callbacks for handling payment lifecycle events
4. **Field Setup**: Mounts the required full name, email, billing address, tax info, and expiry date fields
...
### Boletobancario not eligible

- Verify `testBuyerCountry` is set to "BR"
- Check that Boletobancario is enabled for the merchant in PayPal account settings

### Validation fails

- Ensure all fields are properly mounted
