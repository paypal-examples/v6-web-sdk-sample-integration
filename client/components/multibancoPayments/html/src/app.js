  ...
  multibancoButton.addEventListener("click", async () => {
    try {
      ...
      if (isValid) {
        ...
        await multibancoCheckout.start({ presentationMode: "popup" }, createOrder());
      } else {
...
async function getOrder(orderId) {
  try {
    const response = await fetch(
      `/paypal-api/checkout/orders/get-order/${orderId}`,
      {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      },
...
async function getBrowserSafeClientId() {
  const response = await fetch("/paypal-api/general/client-id", {
    method: "GET",
  });
