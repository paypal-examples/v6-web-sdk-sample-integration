async function onPayPalWebSdkLoaded() {
  try {
    ...
    const isEligible = paymentMethods.isEligible("alfamart");
    if (isEligible) {
      setupAPMPayment(sdkInstance);
    } else {
      showMessage({ text: "Alfamart is not eligible. Please ensure your buyer country is Indonesia and currency is IDR.", type: "error" });
    }
  } catch (error) {
...
function setupAPMPayment(sdkInstance) {
  try {
    const apmCheckout = sdkInstance.createAlfamartOneTimePaymentSession({
...
function setupButtonHandler(apmCheckout) {
  const apmButton = document.querySelector("#alfamart-button");
