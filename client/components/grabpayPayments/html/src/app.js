...
(async () => {
  ...
  const isGrabpayEligible = paymentMethods.isEligible("grabpay");
  if (isGrabpayEligible) {
    const createOrder = buildCreateOrderFunction(currencyCode);
    const grabpayCheckout = sdkInstance.createGrabpayOneTimePaymentSession({
      onApprove,
      onCancel,
      onError,
    });
    const fullnameField = grabpayCheckout.createPaymentFields({
      type: "name",
      value: fullnameValue,
    });
    document.querySelector("#grabpay-full-name").appendChild(fullnameField);
    async function onClick() {
      try {
        const valid = await grabpayCheckout.validate();
        if (valid) {
          await grabpayCheckout.start(
    ...
    const grabpayButton = document.querySelector("#grabpay-button");
    grabpayButton.removeAttribute("hidden");
    grabpayButton.addEventListener("click", onClick);
  }
})();
