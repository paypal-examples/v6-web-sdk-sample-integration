async function onPayPalWebSdkLoaded() {
  try {
    ...
    const sdkInstance = await window.paypal.createInstance({
      clientId,
      testBuyerCountry: "US",
      components: ["pyusd-payments"],
    });
    const paymentMethods = await sdkInstance.findEligibleMethods({
      currencyCode: "USD",
    });
    const isEligible = paymentMethods.isEligible("pyusd");
    if (isEligible) {
      setupPayment(sdkInstance);
    } else {
      showMessage({
        text: "PYUSD is not eligible. Please ensure your buyer country is United States and currency is USD.",
        type: "error",
      });
    }
  } catch (error) {
...
function setupPayment(sdkInstance) {
  try {
    const apmCheckout = sdkInstance.createPyusdOneTimePaymentSession({
...
function setupPaymentFields(apmCheckout) {
  ...
  document.querySelector("#pyusd-full-name").appendChild(fullNameField);
}

function setupButtonHandler(apmCheckout) {
  const apmButton = document.querySelector("#pyusd-button");
